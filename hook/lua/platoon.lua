...
Platoon = Class(SCTAAIPlatoon) {
    ...
    ManagerFactoryAssistAISCTA = function(self)
        ...
        while eng and not eng.Dead and aiBrain:PlatoonExists(self) do
            ...
            eng.Escorting = true
          local Escort = self.FactoryTAAssist(eng, aiBrain, beingBuilt, assistData.AssistRange)
    ...
    FactoryTAAssist = function(eng, aiBrain, category, data)
        local FactoryAssist = aiBrain:GetUnitsAroundPoint(category - categories.TECH1, eng:GetPosition(), data, 'Ally')
        ...
        ManagerEngineerAssistAISCTA = function(self)
            ...
            eng.Escorting = true
              local Escort = self.EngineerTAAssist(eng, aiBrain, beingBuilt, assistData.AssistRange)
        ...
        EngineerTAAssist = function(eng, aiBrain, category, data)
            local EngineerAssist = aiBrain:GetUnitsAroundPoint((categories.ENGINEER * categories.LAND - categories.FIELDENGINEER) + categories.STRUCTURE, eng:GetPosition(), data, 'Ally')
    ...
    EngineerBuildAISCTACommand = function(self)
        ...
        --if eng:TestToggleCaps('RULEUTC_CloakToggle') then
            --eng:EnableUnitIntel('ToggleBit8', 'Cloak')
        --end
        ---eng:SetScriptBit('RULEUTC_CloakToggle', true)
        ---WaitSeconds(10)
        --DUNCAN - added
        if eng:IsUnitState('Building') then
           return
        ...
        SCTAStrikeForceAI = function(self)
            ...
            while aiBrain:PlatoonExists(self) do
                ...
                if not target or target.Dead then
                    ...
                    if target then
                        threat = target:GetPosition()
                        else
                        threat = AIUtils.RandomLocation(self:GetPlatoonPosition()[1],self:GetPlatoonPosition()[3])
                    end
                    if Artillery > 0 then
                        --WaitSeconds(0.25)
                        if not self.ArtilleryThread then
                        self.ArtilleryThread = self:ForkThread(self.PlatoonHandle.SCTAStrikeForceArtillery, threat, Artillery)
                        end
                    end
                    if AntiAir > 0 then
                        if not self.AntiAirThread then
                            self.AntiAirThread = self:ForkThread(self.PlatoonHandle.SCTAStrikeForceAntiAir, threat)
                        end
                        --WaitSeconds(0.25)
                    end
                    ...
                    if target and not target.Dead and not self.Move then
                        WaitSeconds(10)
                        --self:Stop()
                        local direction = TAutils.TAGetDirectionInDegrees(self:GetPlatoonPosition(), threat)
                        IssueFormMove( 'Attack', threat, 'AttackFormation', direction )
                        if oldThreat == threat then
                            self.Move = true
                        end
                                    oldThreat = threat
                                else
                                    ...
                                    coroutine.yield(11)
                                    self:Stop()
                                    if self.ArtilleryThread then
                                        KillThread(self.ArtilleryThread)
                                        self.ArtilleryThread = nil
                                    end
                                    if self.AntiAirThread then
                                        KillThread(self.AntiAirThread)
                                        self.AntiAirThread = nil
                                    end
                                    local direction = TAutils.TAGetDirectionInDegrees(self:GetPlatoonPosition(), (v))
                                ---self:Stop()
                                IssueFormMove( self:GetPlatoonUnits(), (v), 'GrowthFormation', direction )
                                WaitSeconds(7)
                            end
                    end
                end
    ...
    SCTAStrikeForceArtillery = function(self, threat, squad)
        local aiBrain = self.PlatoonHandle:GetBrain()
        local armyIndex = aiBrain:GetArmyIndex()
        ...
        targetArt = AIUtils.AIFindBrainTargetInRange( aiBrain, self, 'Artillery', 25, atkPriArt, aiBrain:GetCurrentEnemy())
        WaitSecond(5)
    ...
    SCTAStrikeForceAntiAir = function(self, threat)
        local aiBrain = self.PlatoonHandle:GetBrain()
        local armyIndex = aiBrain:GetArmyIndex()
        local categoryListA = {}
        local atkPriA = {}
        ...
        while aiBrain:PlatoonExists(self) do
                WaitSecond(5)
                targetAir = AIUtils.AIFindBrainTargetInRange( aiBrain, self, 'Scout', 30, atkPriA, aiBrain:GetCurrentEnemy())
                WaitSecond(5)
    ...
    SCTAExperimental = function(self)
        ...
        while aiBrain:PlatoonExists(self) do
            target = self:FindClosestUnit('Attack', 'Enemy', true, categories.EXPERIMENTAL + categories.COMMAND)
                if not target then
    ...
    BomberAISCTA = function(self)
        ...
        local target
        while aiBrain:PlatoonExists(self) do
            local basePosition = aiBrain:FindClosestBuilderManagerPosition(self:GetPlatoonPosition())
    ...
    SCTAReclaimAI = function(self)
        ...
        while brain:PlatoonExists(self) do
            ...
            local SCTAReclaim = ents[1]:GetPosition()
            ...
            --WaitSeconds(1)
                coroutine.yield(11)
                return self:PlatoonDisbandTA()
            elseif self.PlatoonData.Layer and AIAttackUtils.CanGraphAreaToSCTA(pos, SCTAReclaim, self.PlatoonData.Layer) then
            ---IssueAggressiveMove({eng}, ents:GetPosition())
            ---self:MoveToLocation(ents:GetPosition(), false)
                local reclaimtarget = AIUtils.RandomLocation(SCTAReclaim[1], SCTAReclaim[3])
                self:AggressiveMoveToLocation(reclaimtarget, 'Support')
                coroutine.yield(51)
                local basePosition = brain.BuilderManagers[self.PlatoonData.LocationType].Position
                self:MoveToLocation(AIUtils.RandomLocation(basePosition[1],basePosition[3]), false)
                return self:PlatoonDisbandTA()
